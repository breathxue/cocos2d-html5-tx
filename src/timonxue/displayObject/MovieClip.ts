...
module tx {
     ...
     */
    export class MovieClip extends tx.InteractiveSprite implements IMouseTouchEvent{
        private mTexture:cc.SpriteFrame;
...
//        private mSounds:Vector.<Sound>;
        ...
        constructor(textures:cc.SpriteFrame[], fps:number = 12) {
            super();
            if (textures.length > 0) {
                this.mTexture = textures[0];
                this.mFrameScript = [];
                super.initWithSpriteFrame(textures[0]);
        ...
        private _init(textures:cc.SpriteFrame[], fps:number):void {
            if (fps <= 0) {
...
//            this.mSounds = new Vector. < Sound > (numFrames);
        ...
        public addFrame(texture:cc.SpriteFrame, sound:any = null, duration:number = -1):void {
            this.addFrameAt(this.getNumFrames(), texture, sound, duration);
        }

        /** Adds a frame at a certain index, optionally with a sound and a custom duration. */
        public addFrameAt(frameID:number, texture:cc.SpriteFrame, sound:any = null, duration:number = -1):void {
...
//        this.mSounds.splice(frameID, 1);
        ...
        public getFrameTexture(frameID:number):cc.SpriteFrame {
            if (frameID < 0 || frameID >= this.getNumFrames()) {
                trace("Invalid frame id");
            }
        ...
        public setFrameTexture(frameID:number, texture:cc.SpriteFrame):void {
            if (frameID < 0 || frameID >= this.getNumFrames()) {
                trace("Invalid frame id");
            }
...
//        }
        ...
        public advanceTime(passedTime:number):void {
            ...
            var hasCompleteListener:Boolean = this._ed.hasEventListener(tx.Event.COMPLETE);
...
//            if (this.mSounds[this.mCurrentFrame]) this.mSounds[this.mCurrentFrame].play();
        ...
        public getCurrSpriteFrame():cc.SpriteFrame
        {
        ...
        private _getTexture():cc.SpriteFrame {
            return this.mTexture;
        }

        private _setTexture(value:cc.SpriteFrame):void {
            if (value == null) {
                trace("Texture cannot be null");
            }
            else if (value != this.mTexture) {
                this.mTexture = value;
                super.setDisplayFrame(this.mTexture);
            }
        }
    }
}