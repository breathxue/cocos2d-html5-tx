...
module tx{
     ...
     */
    export class ImageViewMovieClip extends WidgetEx implements IMouseTouchEvent {
        private mTexture:string;
        private mTextures:string[];
...
//        private mSounds:Vector.<Sound>;
        ...
        public iv:ccs.ImageView;

        constructor(iv:ccs.ImageView, fps:number = 12) {
            super(iv);
            if (fps <= 0) {
                trace("Invalid fps: " + fps);
            }
            this.iv = <ccs.ImageView>this.widget;
            var headName:string = this.iv._textureFile;
            var plistName:string;
            var mmtextures:string[];
            //如果是spriteFrame
...
//            this.mSounds = new Vector. < Sound > (numFrames);
        ...
        public addFrame(texture:string, sound:any = null, duration:number = -1):void {
            this.addFrameAt(this.getNumFrames(), texture, sound, duration);
        }

        /** Adds a frame at a certain index, optionally with a sound and a custom duration. */
        public addFrameAt(frameID:number, texture:string, sound:any = null, duration:number = -1):void {
...
//        this.mSounds.splice(frameID, 1);
        ...
        public getFrameTexture(frameID:number):string {
            if (frameID < 0 || frameID >= this.getNumFrames()) {
                trace("Invalid frame id");
            }
        ...
        public setFrameTexture(frameID:number, texture:string):void {
            if (frameID < 0 || frameID >= this.getNumFrames()) {
                trace("Invalid frame id");
            }
...
//                    if (sound) sound.play();
        ...
        public getTotalTime():number {
            var numFrames:number = this.mTextures.length;
...
//            if (this.mSounds[this.mCurrentFrame]) this.mSounds[this.mCurrentFrame].play();
        ...
        private _setTexture(value:string):void {
            if (value == null) {
